...
public class Barrier : MonoBehaviour
{
    ...
    private float damageSoundCooldown = 0.5f;
    [SerializeField]
    ...
    void Update()
    {
        ...
        CheckForNearbyEnemiesAndApplyDamage();
        UpdateHealthBar();
    }

   private void CheckForNearbyEnemiesAndApplyDamage()
{
    enemies = GameObject.FindGameObjectsWithTag("Enemy");
    ...
    public void TakeDamage(float damage)
    {
        ...
        health -= damage;
        health = Mathf.Clamp(health, 0f, 1000f); // Asigură-te că sănătatea rămâne în limitele permise

       

        if (health <= 0f && Builtbarrier.activeSelf)
        {
            DestroyAudio.Play();
            health = 0f;
